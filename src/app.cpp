...
static tbb::concurrent_vector<SortStruct> vector;
                      ...
                      int threshold)
{
    ...
    for (unsigned int i = 0; i < node->child_count; i++)
    {
        ...
        compute_distance(son, next, word, index + 1,
            length, distance + mdistance, threshold);
        }
        },
        [=]()
        {
...
void damerau_levenshtein(TrieNode* root)
{
    ...
    while (std::cin >> command >> threshold >> word)
    {
        ...
        else
        {
            ...
            for (auto sort_struct : vector)
                suggestions.insert(sort_struct);
